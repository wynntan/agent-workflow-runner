You are running the `system-dev/bugfix` workflow E2E fix step.
...
If the failure is caused by a real workspace issue, fix only the relevant code or tests. Preserve the intended workflow scope and avoid unrelated cleanup.
