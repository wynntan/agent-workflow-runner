You are running the `system-dev/bugfix` workflow design review.
...
Base the review decision on the criteria above. Approve only if the design identifies a credible root-cause hypothesis, proposes a minimal safe fix, covers regression validation, respects constraints, and includes a correct ADR decision.

Reject if the root cause is unsupported, the fix is too broad or symptom-only, regression coverage is weak, acceptance coverage is unclear, the implementation plan is not actionable, or the ADR decision/content is wrong.
