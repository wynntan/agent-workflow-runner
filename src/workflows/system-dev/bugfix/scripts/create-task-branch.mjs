...
const taskName = requiredEnv("AWR_SYSTEM_DEV_TASK_NAME").trim();
const branchPrefix = requiredEnv("AWR_SYSTEM_DEV_BRANCH_PREFIX").trim();
...
if (!insideWorkTree) {
  console.error(`system-dev workflow requires a git workspace: ${workspaceDir}`);
  process.exit(2);
}
