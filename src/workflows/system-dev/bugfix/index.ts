...
const workflowId = "system-dev/bugfix";
const workflowName = "System Development Bugfix";
const workflowSummary = "不具合報告を入力に、再現観点、原因調査、修正、検証、E2E、handoff、PR作成を実行するbugfix workflow。";
const inputLabel = "Bug Report";
const inputFileVar = "bug_report_file";
const inputTextVar = "bug_report";
const inputTemplateFile = "bug-report-template.md";
const guidance = [
  "Treat reproduction, expected behavior, actual behavior, and regression coverage as first-class acceptance criteria.",
  "Prefer the smallest targeted fix that addresses the root cause.",
  "Do not broaden the scope into unrelated cleanup unless necessary for the bugfix.",
  "When reproduction is impossible, document the reason and implement only if the root cause is still well supported by evidence."
].join("\n");
const implementSummary = "Implement the smallest safe fix for the diagnosed root cause, and add or update regression coverage where appropriate.";
...
export const bugfixWorkflow: WorkflowDefinition = {
  ...
  variables: [
    {
      name: "task_name",
      summary: "作業branch名の元になるタスク名。bugfix/<slug> のbranchをorigin/dev最新から作成する。",
      type: "string",
      required: true
    },
    {
      name: inputFileVar,
      summary: "不具合報告Markdown file path。再現手順が長い場合はこちらを推奨。",
      type: "string",
      default: ""
    },
    {
      name: inputTextVar,
      summary: "短い不具合報告本文。bug_report_fileが空の場合に使う。",
      type: "string",
      default: ""
    },
    {
  ...
  async run(ctx) {
    ...
    await ctx.shell({
      ...
      summary: "最新のorigin/devを取得し、devからtask_name由来のbugfix branchを作成する。",
      env: {
        AWR_SYSTEM_DEV_TASK_NAME: taskName,
        AWR_SYSTEM_DEV_BRANCH_PREFIX: "bugfix"
      },
