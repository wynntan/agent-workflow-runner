You are running the `system-dev/ui-change` workflow implementation review.
...
Base the review decision on the criteria above. Approve only if the implementation covers target states, interaction behavior, accessibility basics, responsive risks, and follows the existing design system conventions.

Reject if important UI states are missing, accessibility or responsive behavior is harmed, unrelated redesign is introduced, validation gaps are not documented, ADR-required work is mishandled, or correctness risk is obvious.
