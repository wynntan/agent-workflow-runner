You are running the `system-dev/ui-change` workflow validation fix step.
...
Inspect the validation logs referenced by `validation.json`. Fix only the failures from lint, typecheck, build, or unit test. Preserve the approved UI behavior, accessibility considerations, and scope. Do not commit workspace changes.
