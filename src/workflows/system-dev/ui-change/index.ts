...
const workflowId = "system-dev/ui-change";
const workflowName = "System Development UI Change";
const workflowSummary = "UI変更要求を入力に、画面状態、アクセシビリティ、レスポンシブ、検証、E2E、PR作成を扱うui change workflow。";
const inputLabel = "UI Change Requirement";
const inputFileVar = "ui_requirement_file";
const inputTextVar = "ui_requirement";
const inputTemplateFile = "ui-requirement-template.md";
const guidance = [
  "Treat user-visible behavior, layout states, accessibility, responsive behavior, and visual regression risk as first-class concerns.",
  "Follow the repository's existing design system and component conventions.",
  "Avoid broad visual redesigns unless explicitly requested.",
  "When automated browser validation is unavailable, document manual visual checks in validation and handoff."
].join("\n");
const implementSummary = "Implement the requested UI change, covering expected states, accessibility considerations, and responsive behavior within the existing design conventions.";

export const uiChangeWorkflow: WorkflowDefinition = {
  id: workflowId,
  name: workflowName,
  ...
  variables: [
    {
      name: "task_name",
      summary: "作業branch名の元になるタスク名。ui-change/<slug> のbranchをorigin/dev最新から作成する。",
      type: "string",
      required: true
    },
    {
      name: inputFileVar,
      summary: "UI変更要件Markdown file path。画面状態や参考情報が多い場合はこちらを推奨。",
      type: "string",
      default: ""
    },
    {
      name: inputTextVar,
      summary: "短いUI変更要件本文。ui_requirement_fileが空の場合に使う。",
      type: "string",
      default: ""
    },
    {
  ...
  async run(ctx) {
    ...
    await ctx.shell({
      ...
      summary: "最新のorigin/devを取得し、devからtask_name由来のui-change branchを作成する。",
      env: {
        AWR_SYSTEM_DEV_TASK_NAME: taskName,
        AWR_SYSTEM_DEV_BRANCH_PREFIX: "ui-change"
      },
