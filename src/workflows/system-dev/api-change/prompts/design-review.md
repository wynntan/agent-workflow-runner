You are running the `system-dev/api-change` workflow design review.
...
Base the review decision on the criteria above. Approve only if the design clearly covers API contract before/after, compatibility/versioning/migration impact, affected docs/schemas/tests/clients, validation checks, and includes a correct ADR decision.

Reject if compatibility is unclear, contract changes are underspecified, schema/docs/test impact is missing, validation is weak, the implementation plan is not actionable, or the ADR decision/content is wrong.
