You are running the `system-dev/api-change` workflow validation fix step.
...
Inspect the validation logs referenced by `validation.json`. Fix only the failures from lint, typecheck, build, or unit test. Preserve the approved API contract and compatibility requirements. Do not commit workspace changes.
