...
const workflowId = "system-dev/refactor";
const workflowName = "System Development Refactor";
const workflowSummary = "リファクタリング目標を入力に、振る舞い維持の条件を固定し、設計、変更、検証、E2E、handoff、PR作成を実行するrefactor workflow。";
const inputLabel = "Refactor Goal";
const inputFileVar = "refactor_goal_file";
const inputTextVar = "refactor_goal";
const inputTemplateFile = "refactor-goal-template.md";
const guidance = [
  "External behavior must remain unchanged unless the input explicitly says otherwise.",
  "Identify safety rails before editing: tests, snapshots, type checks, or manual verification.",
  "Keep the refactor scoped. Avoid opportunistic feature work.",
  "Call out any behavior change risk clearly in design, review, and handoff."
].join("\n");
const implementSummary = "Refactor the target area while preserving external behavior, keeping changes scoped to the stated goal.";
...
export const refactorWorkflow: WorkflowDefinition = {
  ...
  variables: [
    {
      name: "task_name",
      summary: "作業branch名の元になるタスク名。refactor/<slug> のbranchをorigin/dev最新から作成する。",
      type: "string",
      required: true
    },
    {
      name: inputFileVar,
      summary: "リファクタリング目標Markdown file path。対象や制約が長い場合はこちらを推奨。",
      type: "string",
      default: ""
    },
    {
      name: inputTextVar,
      summary: "短いリファクタリング目標本文。refactor_goal_fileが空の場合に使う。",
      type: "string",
      default: ""
    },
    {
  ...
  async run(ctx) {
    ...
    await ctx.shell({
      ...
      summary: "最新のorigin/devを取得し、devからtask_name由来のrefactor branchを作成する。",
      env: {
        AWR_SYSTEM_DEV_TASK_NAME: taskName,
        AWR_SYSTEM_DEV_BRANCH_PREFIX: "refactor"
      },
