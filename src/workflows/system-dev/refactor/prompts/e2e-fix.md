You are running the `system-dev/refactor` workflow E2E fix step.
...
If the failure is caused by a real workspace issue, fix only the relevant code or tests. Preserve external behavior and avoid unrelated cleanup.
