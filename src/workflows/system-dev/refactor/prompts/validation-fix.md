You are running the `system-dev/refactor` workflow validation fix step.
...
Inspect the validation logs referenced by `validation.json`. Fix only the failures from lint, typecheck, build, or unit test. Preserve external behavior and keep the refactor scoped. Do not commit workspace changes.
