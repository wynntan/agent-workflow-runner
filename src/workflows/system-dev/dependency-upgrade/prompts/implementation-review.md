You are running the `system-dev/dependency-upgrade` workflow implementation review.
...
Base the review decision on the criteria above. Approve only if the requested dependencies and lockfiles changed as planned, unrelated dependency churn is avoided, migration changes are scoped, and validation impact is clear.

Reject if unexpected dependencies changed, lockfiles are inconsistent, migration risk is unaddressed, the implementation conflicts with the package manager strategy, ADR-required work is mishandled, or correctness risk is obvious.
