You are running the `system-dev/dependency-upgrade` workflow design review.
...
Base the review decision on the criteria above. Approve only if the design uses the repo package manager, scopes dependency changes correctly, covers lockfile and migration impact, has a concrete validation plan, and includes a correct ADR decision.

Reject if unrelated dependency churn is likely, migration/breaking-change risk is underexplained, validation is weak, the implementation plan is not actionable, or the ADR decision/content is wrong.
