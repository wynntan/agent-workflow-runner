You are running the `system-dev/dependency-upgrade` workflow E2E fix step.
...
If the failure is caused by a real workspace issue, fix only the relevant code, tests, dependency metadata, or lockfile changes. Keep the fix scoped to the approved dependency upgrade.
