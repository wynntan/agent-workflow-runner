...
const workflowId = "system-dev/dependency-upgrade";
const workflowName = "System Development Dependency Upgrade";
const workflowSummary = "依存関係更新要求を入力に、対象依存・互換性・lockfile・検証・E2E・PR作成を管理するdependency upgrade workflow。";
const inputLabel = "Dependency Upgrade Request";
const inputFileVar = "upgrade_request_file";
const inputTextVar = "upgrade_request";
const inputTemplateFile = "upgrade-request-template.md";
const guidance = [
  "Identify target dependencies, current versions, requested versions or ranges, and lockfile impact.",
  "Prefer package-manager-native commands and preserve the repository's package manager.",
  "Check migration notes or breaking changes when the input provides references or the repo contains relevant docs.",
  "Do not upgrade unrelated dependencies unless needed by the requested upgrade."
].join("\n");
const implementSummary = "Upgrade the requested dependencies, update lockfiles as needed, and keep unrelated package changes out of scope.";

export const dependencyUpgradeWorkflow: WorkflowDefinition = {
  id: workflowId,
  name: workflowName,
  ...
  variables: [
    {
      name: "task_name",
      summary: "作業branch名の元になるタスク名。dependency-upgrade/<slug> のbranchをorigin/dev最新から作成する。",
      type: "string",
      required: true
    },
    {
      name: inputFileVar,
      summary: "依存関係更新要求Markdown file path。対象packageが複数ある場合はこちらを推奨。",
      type: "string",
      default: ""
    },
    {
      name: inputTextVar,
      summary: "短い依存関係更新要求本文。upgrade_request_fileが空の場合に使う。",
      type: "string",
      default: ""
    },
    {
  ...
  async run(ctx) {
    ...
    await ctx.shell({
      ...
      summary: "最新のorigin/devを取得し、devからtask_name由来のdependency-upgrade branchを作成する。",
      env: {
        AWR_SYSTEM_DEV_TASK_NAME: taskName,
        AWR_SYSTEM_DEV_BRANCH_PREFIX: "dependency-upgrade"
      },
